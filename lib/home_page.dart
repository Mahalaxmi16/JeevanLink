...
import 'package:jeevanlink/notifications_page.dart';
import 'package:jeevanlink/all_requests_page.dart';
...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _fetchUserProfile() async {
    try {
      // First, check if user is logged in via SharedPreferences (Donor)
      final prefs = await SharedPreferences.getInstance();
      final userId = prefs.getString('user_id');
      final userRole = prefs.getString('user_role');

      if (userId != null && userRole == 'donor') {
        // Donor login - fetch profile using stored user_id
        ...
        // Hospital login - use Supabase Auth
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error loading profile. Please login again.')),
        );
        setState(() {
          _isLoading = false;
        });
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          if (userRole == 'donor') NotificationBell(userId: _userProfile!['id']),
        ],
      ...
      child: ListView(
        ...
        children: <Widget>[
          ...
          ListTile(
            ...
            onTap: () async {
              // Clear SharedPreferences (for donors)
              final prefs = await SharedPreferences.getInstance();
              await prefs.clear();

              // Sign out from Supabase Auth (for hospitals)
    ...
    _notificationSubscription = Supabase.instance.client
      ...
      if (mounted) {
        final filteredNotifications = notifications.where((notification) =>
          notification['donor_id'] == widget.userId && notification['status'] == 'pending'
        ).toList();
...
class _EnhancedDonorDashboardState extends State<EnhancedDonorDashboard> {
  ...
  final Set<String> _knownNotificationIds = {}; // Track seen notifications
  ...
  Future<void> _loadInitialNotifications() async {
    try {
      ...
      if (mounted) {
        // Fix: Explicitly filter out any 'fulfilled' requests just in case
        final active = data.where((n) => n['status'] != 'fulfilled').toList();

        // Populate known IDs so we don't notify for existing ones
        for (var n in active) {
          _knownNotificationIds.add(n['id'].toString());
        }

        setState(() {
          _notifications = List<Map<String, dynamic>>.from(active);
          _pendingRequests = active.length;
        });
      }
    } catch (e) {
  ...
  void _setupNotificationStream() {
    _notificationStream = Supabase.instance.client
      ...
      if (mounted) {
        // Fix: Ensure strict filtering for active requests
        final filtered = allNotifications.where((n) =>
          n['donor_id'] == widget.userProfile['id'] &&
          (n['status'] == 'pending' || n['status'] == 'accepted') &&
          n['status'] != 'fulfilled'
        ).toList();

        // Check for NEW notifications to show alert
        for (var n in filtered) {
          final id = n['id'].toString();
          if (!_knownNotificationIds.contains(id)) {
            _knownNotificationIds.add(id);
            // Only notify if it's a pending request (new match)
            if (n['status'] == 'pending') {
              NotificationService().showNotification(
                title: 'New Blood Request Match!',
                body: 'A hospital nearby needs your help. Tap to view.',
              );
            }
          }
        }
  ...
  Future<void> _acceptRequest(BuildContext context, String requestId, String notificationId) async {
    final donorId = widget.userProfile['id'];
    try {
                              ...
                              Center(
                                child: FutureBuilder(
                                  ...
                                  builder: (context, responseSnapshot) {
                                    ...
                                    if (responseId == null) return const SizedBox();

                                    // Fix: Check if THIS request is the one being tracked
                                    final isTrackingThis = _currentlyTrackingResponseId == responseId;
                                    final isTrackingAny = _currentlyTrackingResponseId != null;
                                    ...
                                    if (isTrackingThis) {
                                    ...
                                    } else if (isTrackingAny) {
                                      return Text(
                                        'Tracking another request...',
                                      ...
                                      return ElevatedButton.icon(
                                        ...
                                        onPressed: () => _startLiveTracking(responseId),
                                        style: ElevatedButton.styleFrom(
                                          backgroundColor: Colors.blue,
                                          foregroundColor: Colors.white,
                                          shape: RoundedRectangleBorder(
...
class _EnhancedHospitalDashboardState extends State<EnhancedHospitalDashboard> {
  ...
  final Set<String> _knownResponseIds = {}; // Track seen responses
  ...
  void _listenToRequests() async {
    ...
    try {
      ...
      setState(() {
        _requests = initialRequests as List<Map<String, dynamic>>;
      });
    } catch (e) {
  ...
  void _setupResponseListener() {
    // Listen for NEW accepted responses to my requests
    _responsesSubscription = Supabase.instance.client
        .from('donor_responses')
        .stream(primaryKey: ['id'])
        .listen((data) async {
      if (mounted) {
        // Filter for 'accepted' responses
        final accepted = data.where((r) => r['response_status'] == 'accepted').toList();

        for (var response in accepted) {
          final id = response['id'].toString();

          // If we haven't seen this response before
          if (!_knownResponseIds.contains(id)) {
